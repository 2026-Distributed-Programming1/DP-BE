...
> - 추가 완료(2026-06-04): `ApiSpec.md` 전 컨트롤러(34개) 명세 완성 · 청약신청/청약서/부활신청/성과급요청 목록·단건 GET 구현 · `ApplicationType` enum 도입(`POLICY`/`INSURANCE`) · `underwritings.application_type` DB 컬럼 추가(V4 migration) · 인수심사 완료 시 신청번호 직접 조회 + `applyUnderwritingResult()` 엔터티 위임으로 상태 갱신 보강 · 교육 제반 등록 전 연결 교육 계획 `APPROVED` 검증(`requireApproved()`) · 시드 데이터 보강.
