...
> - 추가 완료: Flyway 도입 · HTTP 세션 로그인 · Spring Session JDBC · `auth_users` 기반 role · 고객 직접 회원가입 · 주요 고객 데이터 서비스 소유권 검증.
> - **다음 작업**: 고객 검색/상세 API · enum/options API · 공통 에러 응답 보강 · 문의 customer_id 연결 · 파일/S3 정책.
...
- **권한/소유권**: `role`은 도메인 엔터티가 아니라 `auth_users.role`에서 관리한다. 공통 interceptor는 로그인 여부만 검사하고, 고객 데이터 소유권은 Service 계층에서 `AuthAccessService`로 검증한다. `CUSTOMER`는 `linked_customer_id`로 연결된 본인 고객 데이터만, `STAFF`/`ADMIN`은 업무상 전체 접근을 허용한다.
...
**F. 인증 변경 시 주의.** `/api/auth/**`를 제외한 `/api/**`는 로그인 세션이 필요하다. 고객용 API를 추가할 때는 단순 role 검사보다 `AuthAccessService`로 데이터 소유권을 검증한다. 문의처럼 아직 `customer_id`가 없는 테이블은 소유권 검증 전 migration으로 고객 식별자를 먼저 추가한다.
