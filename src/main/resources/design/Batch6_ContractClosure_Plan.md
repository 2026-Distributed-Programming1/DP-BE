...
> **원칙(메인 `ApiMigrationPlan.md` 부록 A 그대로)**: UC 신규 전환(Controller/Service/Repository/DTO) + surrogate-PK(id) + 업무번호 id-파생(저장형). 레거시 `old/`는 무수정 공존. 한 테이블 한 writer(전환 후 해당 Runner는 콘솔에서 은퇴).
> **API 설계**: 콘솔 단계 순서를 베끼지 말 것. Runner의 비즈니스 규칙·검증·분기만 Service로 이관, 프론트 주도·무상태. 연쇄는 단계마다 1 엔드포인트·자기 테이블 1행 저장, 부모는 URL 업무키로 DB에서 셸 복원(메인 §claim 패턴과 동일).

---

## 0. 대상 정리

| UC | 엔터티 | DAO(정답지) | Runner(은퇴 대상) | 테이블 | 도메인(feature) |
|---|---|---|---|---|---|
| 보험 해지 | `contract/entity/Cancellation` | `CancellationDAO` | `InsuranceCancellationRunner` | cancellations | contract |
| 해약 환급금 산출 | `payment/entity/RefundCalculation` | `RefundCalculationDAO` | `RefundCalculationRunner` | refund_calculations | payment |
| 해약 환급금 지급 | `payment/entity/RefundPayment` | `RefundPaymentDAO` | `RefundPaymentRunner` | refund_payments | payment |
| 해약 환급 내역 조회 | (위 둘 조회) | — | `RefundListRunner` | refund_calculations/payments | payment |
| 계약 통계 관리 | `contract/entity/ContractStatistics` | `ContractStatisticsDAO` | `ContractStatisticsRunner` | contract_statistics | contract |
| 만기 계약 관리 | `contract/entity/ExpiringContractManagement` | `ExpiringContractManagementDAO` | `ExpiringContractManagementRunner` | expiring_contract_notices | contract |
...
```
- `refund_calculations.cancellation_no → cancellations.cancellation_no`, `refund_payments.refund_no → refund_calculations.refund_no` (+ `refund_payments.cancellation_no`도 보유).
- 계약통계·만기관리는 **독립**(연쇄 아님). 통계는 contracts 집계 스냅샷, 만기는 만기임박 계약 안내.
- 해지·환급·만기는 `contracts`를 부모로 참조 → contract_no는 이미 배치1에서 UNIQUE이므로 FK 그대로 동작.

## 2. PK 파운데이션 ✅ (2026-05-31 완료)

전부 현재 VARCHAR PK → **`id BIGINT AUTO_INCREMENT PRIMARY KEY` 추가 + 업무키 `UNIQUE` 강등**. 엔터티에 `Long id` + getId/setId 추가.

| 테이블 | 업무키 | 접두 | 비고 | 완료 |
|---|---|---|---|:---:|
| cancellations | cancellation_no | `CAN` | 엔터티 `static sequence` 보유 → id-파생으로 전환 | ✅ |
| refund_calculations | refund_no | `RFC` | 동일 | ✅ |
| refund_payments | payment_no | `RPY` | 동일 | ✅ |
| contract_statistics | stats_no | `STA` | 엔터티 전면 재정의(스냅샷용 필드로 교체) + Runner 호환 스텁 유지 | ✅ |
| expiring_contract_notices | notice_no | `EXP` | 엔터티에 id·noticeNo 필드 추가, 기존 필드 유지 | ✅ |

**추가 결정사항**:
- `ContractStatistics` 엔터티: 기존 화면용 필드(filterStartMonth 등)는 Runner 컴파일 호환을 위해 스텁으로 유지, 신규 DB 필드(statsNo·카운트류)를 추가하는 방식으로 재정의.
- `ExpiringContractManagement`: 계약당 N건 허용(contract_no UNIQUE 없음), id-파생 notice_no 신규 도입.

## 3. 도메인별 구현 (메인 §8 + claim 패턴 복제)

각 UC: Repository(`SqlExecutor` 주입, finder + `save()`=INSERT→`executeInsertReturningKey`→업무번호 파생 UPDATE) → Service(`@Transactional`, 조회 readOnly, Runner의 규칙·검증·E1 이관) → DTO(record) → Controller(REST). **참고 구현**: `domain/claim/**`(연쇄·상태가드), `domain/contract/**`(조회), `domain/payment/PaymentRepository`(id-파생 save).
...
- **해지(Cancellation)**: `CancellationRepository`, `CancellationService`, `CancellationController`.
  ...
  - 규칙: 해지 사유·예상환급액 산정은 엔터티 메서드로. 상태 전이(신청→처리)는 엔터티 행위 메서드 사용. `InsuranceCancellationRunner`가 환급계산까지 연쇄했는지 확인 — 했다면 환급 산출은 **별도 엔드포인트**로 분리(무상태).
...
- **추측 금지(claim 1차 실패 교훈)**: 작성 전 ① 대상 엔터티의 실제 게터·생성자, ② schema.sql 실제 컬럼명, ③ 대응 `old/dao/*DAO`의 save/findAll(매핑·CSV 처리 등)을 **눈으로 확인**. 특히 ContractStatistics 필드↔컬럼 불일치, stats/expiring 번호 생성 방식.
- 레거시 `old/dao`·`old/runner`·도메인 `static sequence` 무수정. 전환된 UC의 Runner는 콘솔 메뉴에서 은퇴.
- FK는 업무키(UNIQUE) 문자열 참조 유지 → 재배선 불필요.
- 검증 시드: 해지/환급/만기는 contracts·customers 시드(DataSeeder)에 의존. 통계는 기존 계약 4건으로 집계 확인 가능. 환급 연쇄는 해지부터 API로 생성.
- 완료 후 **메인 `ApiMigrationPlan.md` 부록 A.4 감사표·A.6 배치표·§9 To-Do** 와 **CLAUDE.md 현재 상태/PK 방식 표**를 갱신할 것(이 4개 UC가 마지막 미전환분이라, 끝나면 "전 UC 전환 완료 → DBA 제거 검토" 단계로 이동).