...
🔗 photos: List<DispatchPhoto>     [사진 목록]
- policeRequired / towingRequired: boolean
- notes: String
- transmittedAt: LocalDateTime
🔗 status: DispatchRecordStatus    [상태 «enum»]

+ uploadPhoto(category, photo: DispatchPhoto): void
+ removePhoto(photo): void
+ validateRequired(): boolean
+ transmit(): void
```

## `DispatchPhoto` [현장 출동 사진 메타데이터]

```
- photoId: Long                   [사진 메타데이터 ID]
🔗 recordId: Long                  [출동 기록 ID]
- fileName: String                [파일명]
- filePath: String                [저장 경로]
- fileSize: long                  [파일 크기]
- mimeType: String                [파일 형식]
...
DispatchRecord "1" ◇────▶ "*" DispatchPhoto                 : 사진 보유
