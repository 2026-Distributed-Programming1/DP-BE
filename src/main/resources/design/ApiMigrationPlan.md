...
> **현재 상태 (2026-05-31)**: 패키지 구조 개편 ✅ · 파일럿 2개(계약 조회·보험료 납입) ✅ · Spring `@Transactional` ✅ · 시더 ✅ · surrogate-PK **배치 1(contract+payment) ✅** · **배치 2(claim) 전체 완료 ✅**(PK 파운데이션 7테이블 + 2a 보상 UC 4개 + 2b 사고·출동 UC 2개. Controller/Service/Repository/DTO, 무상태·단계분리, multipart 사진 업로드, 흐름·예외 API 검증 완료 — `Batch2_Claim_Plan.md` §8·§9).
>
> **▶ 다음 작업 (RESUME HERE)**: **배치 3 — consultation 도메인** (상담요청·면담일정·면담기록·제안·인수심사·가입신청·청약·부활, 8테이블). claim과 달리 **PK 파운데이션부터** 함께 해야 한다(스키마 surrogate-PK + `insurance_applications`·`policy_applications`는 INT→AUTO_INCREMENT, 엔터티 `Long id` + UC 전환). 착수 시 `Batch3_Consultation_Plan.md` 세부 계획부터 수립 권장(claim 방식). 작업 규칙은 **`CLAUDE.md` 「전환 작업 원칙」·「전환 구현 체크리스트」** 준수.
...
| **PK 동시성** | **도메인 배치 단위 surrogate-PK 전환 진행 중** | 시퀀스형 업무번호는 DB `id`(AUTO_INCREMENT)에서 파생(저장형). 배치 1·2(PK) 완료, 3~5 예정 (부록 A). 미전환 도메인은 `static sequence` 잔존(레거시 공유). |
...
- [ ] **배치 3: consultation** ◀ 다음 / **4: sales** / **5: education+inquiry+마스터** — UC 신규 전환 + surrogate-PK (부록 A.6)
...
| 5 | education + inquiry + 마스터 | education_plans/preparations/executions(attendances=이미 id), inquiries, actor 마스터(designers·agencies·임직원 8종, id만) |
