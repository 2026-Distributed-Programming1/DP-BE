...
> **현재 상태 (2026-05-31)**: 패키지 구조 개편 ✅ · 파일럿 2개(계약 조회·보험료 납입) ✅ · Spring `@Transactional` ✅ · 시더 ✅ · surrogate-PK **배치 1(contract+payment) ✅** · **배치 2(claim) 전체 완료 ✅** · **배치 3(consultation) 전체 완료 ✅** · **배치 4(sales) 전체 완료 ✅**(PK 파운데이션 6테이블 + 컬럼명 통일 + 4a(모집공고·채용심사·고객등록) + 4b(활동계획+일정) + 4c(영업활동관리·조직평가·성과급). Controller/Service/Repository/DTO, 흐름·예외 API 검증 완료 — `Batch4_Sales_Plan.md`).
>
> **▶ 다음 작업 (RESUME HERE)**: **배치 5 — education + inquiry + 마스터** (education_plans·preparations·executions, inquiries, actor 마스터). 세부 계획은 `Batch5_Education_Plan.md` 수립 권장. 작업 규칙은 **`CLAUDE.md` 「전환 작업 원칙」·「전환 구현 체크리스트」** 준수.
...
- [ ] **배치 5: education+inquiry+마스터** ◀ 다음 — UC 신규 전환 + surrogate-PK (부록 A.6)
