...
> **현재 상태 (2026-05-31)**: 패키지 구조 개편 ✅ · 파일럿 2개(계약 조회·보험료 납입) ✅ · Spring `@Transactional` ✅ · 시더 ✅ · surrogate-PK **배치 1(contract+payment) ✅** · **배치 2(claim) 전체 완료 ✅** · **배치 3(consultation) 전체 완료 ✅** · **배치 4(sales) 전체 완료 ✅** · **배치 5(education+inquiry+마스터) 전체 완료 ✅** (스키마 컬럼명 통일 + PK 파운데이션 13테이블 + 5a(교육계획안) + 5b(교육제반+교육진행) + 5c(문의). Controller/Service/Repository/DTO, 흐름·예외 API 검증 완료).
>
> **▶ 다음 작업 (RESUME HERE)**: **최종 수렴** — 전 UC 전환 완료. `old/`(DBA·DAO·Runner) 삭제 + 업무번호 저장 폐기 → format-on-read + FK를 id로 전환 (§9 참조). 또는 CORS/인증·인가 추가.
