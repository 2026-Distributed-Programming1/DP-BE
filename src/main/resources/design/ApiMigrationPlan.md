...
> **현재 상태 (2026-05-30)**: 패키지 구조 개편 ✅ · 파일럿 2개(계약 조회·보험료 납입) ✅ · Spring `@Transactional` ✅ · 시더 ✅ · surrogate-PK **배치 1(contract+payment) ✅** · **배치 2(claim) PK 파운데이션 ✅**(7테이블 id PK + 엔터티 Long id).
>
> **▶ 다음 작업 (RESUME HERE)**: **배치 2a — claim 보상 UC 신규 전환** (보험금 요청·손해조사·산출·지급). claim 7테이블 id PK·엔터티 `Long id`는 이미 완료됐고, 남은 건 Repository/Service/Controller/DTO 작성뿐. 세부 계획·엔드포인트·주의사항은 **`design/Batch2_Claim_Plan.md`** 에 있다.
...
- [~] **배치 2: claim** — PK 파운데이션 ✅ / **◀ 다음: 2a UC 전환**(보상: 요청·조사·산출·지급) → 2b(사고·출동). 세부: `Batch2_Claim_Plan.md`
- [ ] **배치 3: consultation** / **4: sales** / **5: education+inquiry+마스터** — UC 신규 전환 + surrogate-PK (부록 A.6)
...
| 2 | claim (PK ✅ / UC 전환 중) | accident_reports, dispatches, dispatch_records, claim_requests, damage_investigations, claim_calculations, claim_payments |
