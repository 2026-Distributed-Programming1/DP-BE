...
12. 앱 기동 시 JAR 내부 Flyway migration으로 DB schema를 생성/변경
...
- `src/main/resources/db/migration`
  - Flyway schema migration. `V1__init_schema.sql`이 전체 초기 schema, 이후 버전이 변경분이다.
...
APP_AUTH_BOOTSTRAP_USERNAME=admin
APP_AUTH_BOOTSTRAP_PASSWORD=change-admin-password
APP_AUTH_BOOTSTRAP_DISPLAY_NAME=관리자
SESSION_TIMEOUT=30m
SESSION_COOKIE_NAME=DPBE_SESSION
...
- 최초 관리자 계정 생성이 필요하면 `APP_AUTH_BOOTSTRAP_ENABLED=true`와 `APP_AUTH_BOOTSTRAP_PASSWORD`를 설정한 뒤 배포한다. 계정이 생성된 뒤에는 다시 `false`로 두는 것을 권장한다.
- HTTPS 없이 HTTP로 세션을 쓰므로 `SESSION_COOKIE_SECURE=false`를 사용한다. 프론트와 백엔드가 다른 origin이면 프론트 요청에 credentials 설정이 필요하다.
- Flyway가 전체 schema를 관리하므로 초기 전환 배포 전 EC2에서 `docker compose down -v`로 기존 MySQL volume을 제거한다.
- 기존 DB를 삭제하지 않고 붙일 때만 `SPRING_FLYWAY_BASELINE_ON_MIGRATE=true`를 검토한다. 이번 전환에서는 사용하지 않는다.
- Spring Boot 4에서는 Flyway 자동 실행을 위해 `spring-boot-starter-flyway`가 필요하다. `flyway-core`만 추가하면 앱 기동 시 migration이 실행되지 않는다.
...
- `mysql`
  ...
  - DB 자체만 생성한다.
  - 테이블 생성과 변경은 앱 기동 시 JAR 내부 Flyway migration으로 적용된다.
  - 신규 DB에서는 `V1__init_schema.sql`부터 순서대로 실행된다.
...
로컬:
- 기본 `docker-compose.yml`의 `mysql`만 실행하면 된다. MySQL 컨테이너는 빈 DB만 만들고, 테이블은 앱 기동 시 Flyway가 생성한다.
...
`schema.sql`은 삭제됐고 더 이상 EC2에 업로드하지 않는다. DB schema는 앱 이미지 안의 Flyway migration에서 관리한다.
...
## 10. 인증/Flyway smoke test 기록

2026-06-01 로컬 fresh MySQL volume 기준으로 아래를 확인했다.

- Flyway `V1__init_schema.sql` 자동 실행 성공
- 서버 기동 성공
- `POST /api/auth/signup/customer` 고객 직접 회원가입 200 확인
- `POST /api/auth/login` 고객 로그인 200 및 `DPBE_SESSION` 발급 확인
- `GET /api/auth/me` 고객 세션 조회 200 확인
- 관리자 로그인 200 확인
- 관리자 세션으로 `POST /api/auth/staff-accounts` 직원 계정 발급 200 확인
- 직원 계정 발급 시 `CUSTOMER` role 요청은 400으로 거부 확인
- `./gradlew build -x test` 성공

테스트 중 수정한 이슈:

- Boot 4에서 Flyway migration이 자동 실행되지 않아 `spring-boot-starter-flyway`로 의존성을 변경했다.
- 고객 직접 회원가입 시 임시 `customer_id`가 `VARCHAR(20)`을 초과하지 않도록 조정했다.
