...
> **현재 상태 (2026-06-04)**: 콘솔(Runner) 구조를 Spring REST API 구조로 전환 완료. 최종 수렴도 완료되어 `old/`/`OldMain` 삭제, 업무번호 format-on-read, FK `id` 전환이 반영됨.
...
> - 추가 완료(2026-06-04): `ApiSpec.md` 전 컨트롤러(34개) 명세 완성 · 교육 제반/실행·계약 통계·만기 계약 명세 추가 · 청약신청/청약서/부활신청/성과급요청 목록·단건 GET 구현(Repository `findPage`/`findByNo`/`mapRow`·Service·Controller) · mapRow timestamp 버그 수정(`apply()`/`submit()` → 실제 DB 값으로 복원) · 엔티티 `setAppliedAt`/`setSubmittedAt` setter 추가.
> - 추가 완료(2026-06-04, refactor/logic-reinforcement): `ApplicationType` enum 도입(`POLICY`/`INSURANCE`) · `underwritings.application_type` DB 컬럼 추가(V4 migration) · 인수심사 완료 시 신청번호 직접 조회 + `applyUnderwritingResult()` 엔터티 위임으로 상태 갱신 보강 · 교육 제반 등록 전 연결 교육 계획 `APPROVED` 검증(`requireApproved()`) · 시드 데이터 보강.
> - **다음 작업**: 목록 필터 확장 · 파일/S3 정책 설계.
...
- **`src/main/resources/design/old/ApiMigrationPlan.md`** — API 전환 계획·현황·결정 근거 과거 기록
- **`src/main/resources/design/FrontendIntegrationReview.md`** — 프론트 연동, auth/session, Flyway, customer CRUD 백로그
- **`src/main/resources/design/ApiSpec.md`** — 프론트 병렬 작업용 수동 API 명세와 enum/권한 정책
- **`src/main/resources/design/RoleBasedAccessSmokeTest.md`** — role 기반 접근 제어 200/403 smoke test 시나리오
...
- **2026-06-04**: `V4__add_application_type_to_underwritings.sql` — `underwritings.application_type VARCHAR(20) NULL` 컬럼 추가. `ApplicationType` enum(`POLICY`/`INSURANCE`) 도입으로 인수심사-신청 상태 갱신 경로 명시화.
